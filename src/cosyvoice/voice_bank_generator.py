...
生成的 WAV 内容为固定文本：这是一个有声书音频agent项目，我会用不同的音色朗读这一段测试文本
"""
...
def generate_voice_bank(
    ...
    Args:
        characters: character_analyzer 的输出
        output_dir: 输出目录（如 output-qwen-cosy/{story_name}/）
        force: 是否强制重生成已有音频

    Returns:
        speaker -> audio_path 的映射
    """
...
def _post_process_wav(audio_path: str) -> str:
    ...
    CosyVoice 对参考音频的帧数有最小要求，Qwen3 生成的音频
    可能首尾有大量静音，导致有效帧数不足。
    """
